...
struct nodo* criaNodo(int novoDado){//cria novos nodos para a lista mas não insere
	struct nodo *novoElemento = (struct nodo *)malloc(sizeof(struct nodo));
	novoElemento->dado = novoDado;
	novoElemento->prox =  NULL;
	return novoElemento;
}
struct desc_lista_encadeada * criaDescritor(void){//cria descritores de listas
