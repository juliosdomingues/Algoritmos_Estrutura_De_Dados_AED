...
int somaRecursiva(int vetor[], int tamanho){
	printf("chegou na soma com tamanho %d\n",tamanho);
	if(tamanho == 1){
		printf("chegou no base vai retornar o vetor indice 0\n");
		return vetor[0];
	}
		else{
		printf("chegou else com tam %d\n", tamanho);
		float result = somaRecursiva(vetor,(tamanho-1));	
		printf("voltou da recursao com resultado %f\n",result);
		return (vetor[tamanho-1]+result);
	}
}
